...
| **search_keybind** | **キーバインド検索 (New!)** | 「ドッキングのキーは何？」「空いているキーは？」「Quantum Drive に関するキーを表示して」 |
...
- **Activation Mode 対応**: tap / press / hold / double_tap の色分け表示
  - 長押し (紫)、二度押し (緑) などが一目でわかる
  - ツールチップに「タップ」「長押し」「二度押し」等の activation mode を表示
- **修飾キーチェックボックス**: L-Shift / R-Ctrl / L-Alt 等の組み合わせバインドをワンクリックで確認
- **翻訳連携**: 機能名にゲーム内の日本語翻訳 (global.ini) を使用

### チャットのキーバインド検索

- チャットから `search_keybind` スキルでキーバインドを検索可能
- 「空いているキーは？」「Quantum Drive のキーは？」「ドッキングのキーは何？」などの質問に対応
- 検索結果に activation mode (タップ・長押し・二度押し等) の情報も含む

### API ドキュメント改善

- Claude / Gemini の API ドキュメントに料金目安テーブルと Usage Tier 詳細を追加
