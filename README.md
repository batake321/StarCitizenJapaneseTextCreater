...
> **[StarCitizenJapaneseTextCreater-v1.12.8-win-x64.zip (最新 v1.12.8 バイナリ)](https://github.com/batake321/StarCitizenJapaneseTextCreater/releases/download/v1.12.8/StarCitizenJapaneseTextCreater-v1.12.8-win-x64.zip)**
>
> **[sc_japanese_backup_20260603_032750.zip (翻訳データベース)](https://github.com/batake321/StarCitizenJapaneseTextCreater/releases/download/v1.12.8/sc_japanese_backup_20260603_032750.zip)** — アプリの「インポート (復元)」から取り込めます。

### 🚀 v1.12.8 更新内容

- **キーバインドエディタ: マウスタブ追加**: 左/右クリック・ホイール・サイドボタン・軸をビジュアル表示。色分け・ツールチップ・クリックで割り当て編集。右側にマウスバインド全一覧を表示
- **キーバインドエディタ: ゲームパッド (Xbox) タブ追加**: A/B/X/Y・LB/RB・LT/RT・スティック・D-Pad をビジュアル表示。LB/RB 修飾ボタン対応、クリックで割り当て編集
- **キーバインドエディタ: HOTAS タブ強化**: VKB Gladiator NXT 左右の画像を適切に縮小表示、ボタングループ別バインドテーブル追加。画像クリックで拡大ビューア＋バインド編集パネルを開く
- **ゲームパッド入力コードの正規化**: defaultProfile.xml のプレフィックスなし形式 (`thumbl`, `a` 等) をパース時に正規化 (`gp1_thumbl`, `gp1_a` 等) し、バインド表示・検索・保存の一貫性を確保
- **翻訳エディタ: ワイルドカード検索の修正**: 部分一致チェック時の `vehicle*` 検索がアンカー (`^...$`) なしで動作するよう修正。検索フィールド変更・部分一致トグル時にも即再検索
- **一括置換後のDB自動リフレッシュ**: 用語集の一括置換後に翻訳エディタが常に最新データを再読込するよう修正
- **Claude Opus 4.7 対応**: Opus モデルで非対応の `temperature` パラメータを除外し、翻訳・チャットの API エラーを修正
- 翻訳データベースのバックアップを最新化 (`sc_japanese_backup_20260603_032750.zip` 同梱)
