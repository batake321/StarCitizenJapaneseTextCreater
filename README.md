...
設定タブの **「エクスポート (バックアップ)」** で、データベースの内容を SQL + GZip 圧縮 (`.sql.gz`) で保存できます。

#### エクスポート

全データ（翻訳・用語集・インデックス）を 1 つのファイルに圧縮保存します。

#### インポート

**「インポート (復元)」** ボタンで `.sql.gz` ファイルを読み込みます。インポート時にダイアログで取り込むデータを選択できます:
...
1. アプリの **エクスポート** で `.sql.gz` を `db_backup/` に保存
