...
> **📖 [図解つき 使い方マニュアル](https://batake321.github.io/StarCitizenJapaneseTextCreater/)** — インストールから翻訳の当て方、船舶管理・Web チャットまで一通り解説しています。初めての方はこちらから。
...
- **翻訳データをリセットしたい場合**: WorkDir 内の以下のファイルを削除してからアプリを起動してください。同梱DBがコピーされクリーンな状態になります
  - `translations.db` — 翻訳データ
...
- **アップグレードシミュレーション**: 保有船ごとに使える権利を表示。「Upgrade」で適用 → 連鎖 → 「Undo」。使用中の権利は紫色、使用可は緑色で表示、元船を持たない権利は「使用不可」、連鎖のない船は灰色で区別
- **売却 (melt) のクレジット管理**: 「Sell」で pledge 単位の売却を試算。現在の Store Credit ＋ 売却で得られる Credit（適用中の CCU 額を含む）、失う機体、LTI 喪失警告、Buy Back 残トークンを常時表示。売却した船を起点とする権利は「使用不可 (売却)」に
- **船の由来・保有数・装備**: 各船に「単品 / パック / CCU 適用済 / VIP 特典」の由来と同梱機、同型の保有数 (×N) を表示。パックは「+」で展開。船名のツールチップにメーカー・役割・サイズ・実装状態・装備スロット (武器 / シールド / 量子ドライブ 等) を表示
- **所持船タブの拡張**: My Hangar 同期で所持船リストを自動更新 (手入力分は保護)。各行に保有数・由来・Upgradable・販売状況・購入可能なアップグレード数と「アップグレード」ボタンを追加。バージョン違い (Executive Edition 等) は別の船として扱います
...
- **初期装備の取得**: 船のデフォルトロードアウトをゲームデータから完全に取得（ジンバル配下の武器・ミサイル・ジャンプドライブ・タレット武装を含む）。「装備データを再抽出」ボタンで船・装備データだけを数分で更新できます（初回は実行してください）
- **装備の登録**: 所持コンポーネント（使用可/不可）と船ごとの現在装備を設定できます。未設定のポートは初期装備を表示
- **ゲーム内購入の船**: 所持船の追加時に「ゲーム内購入 (aUEC)」を選ぶと由来として表示され、アップグレード管理にも一覧されます（CCU / melt の対象外）
...
> より詳しい手順と図解は **[使い方マニュアル](https://batake321.github.io/StarCitizenJapaneseTextCreater/)** にまとめています（コモディティ・船舶管理・価格キャプチャ・ミッションの説明もこちらにあります）。
